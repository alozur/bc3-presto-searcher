...
export function parseBc3(bytes: Uint8Array, source: SourceKey, display: string): ImportSnapshot {
  ...
  for(const [idx,line] of records){const lineNo=idx+1; const m=line.match(/~([A-Z])\|([\s\S]*)/); if(!m)continue; const tag=m[1], body=m[2]; const f=body.split('|');
